...
public class SimplePermissionsPlugin
  ...
  public static void registerWith(Registrar registrar) {
    final MethodChannel channel =
        new MethodChannel(registrar.messenger(), "simple_permissions");
    SimplePermissionsPlugin simplePermissionsPlugin =
        new SimplePermissionsPlugin(registrar);
  ...
  private String getManifestPermission(String permission) {
    ...
    switch (permission) {
      ...
      case "WRITE_CONTACTS":
        ...
        break;
      case "GET_ACCOUNTS":
        res = Manifest.permission.GET_ACCOUNTS;
        break;
      default:
        res = "ERROR";
        break;
  ...
  public boolean onRequestPermissionsResult(int requestCode,
    ...
    if (requestCode == 0 && grantResults.length > 0) {
      if (ActivityCompat.shouldShowRequestPermissionRationale(
              registrar.activity(), permission)) {
        // denied
        status = 2;
      } else {
        int selfPerm=ActivityCompat.checkSelfPermission(registrar.context(),
        permission);
        System.out.println("selfperm: " +selfPerm);
        if (selfPerm ==
